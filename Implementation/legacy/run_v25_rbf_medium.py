...
def run_experiment():
    ...
    run_name = f"dge_v25_rbf_med_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # 2. Expand: V25 RBF Medium Configuration (64 Experts)
    print(f"Step 2: Expanding Model (RBF-Bigram Router - 64 Experts)...")
    
    current_d_model = model.d_model
    model.expand_model(
        new_input_dim=current_d_model + 64, # V25: 64 Experts (Medium Capacity)
        new_output_dim=model.token_emb.num_embeddings, 
        router_type='rbf_bigram',
