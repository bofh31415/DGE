...
def run_v12_separate_qkv():
    print(f"\n" + "="*60)
    print(f"🚀 RUNNING V12 SEPARATE QKV: Structural Fix + Clean Boost (LR 1e-2)")
    print("="*60)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_name = f"dge_v12_separate_qkv_{timestamp}"
    ...
    # + Zero Cross Terms (Implicit in patched dge_utils)
    # + Separate QKV (Implicit in patched dge_model)
    print(f"Step 2: Expanding Model (Bias=-4.0, Zero Init, Clean Cross, Sep QKV)...")
    ...
    print(f"✅ V12 Experiment Complete. Logs in {exp_dir}")

if __name__ == "__main__":
    run_v12_separate_qkv()
