...
def run_experiment():
    ...
    run_name = f"dge_v24_strict_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # 2. Expand: V24 Strict Separation Configuration
    # - Bigram Router (Contextual)
    # - Imprint Policy
    # - Gradient Rescue
    # - Hard Threshold (0.05) via STE
    print(f"Step 2: Expanding Model (Bigram Router, Hard Threshold < 0.05)...")
    print("Hypothesis: Hard Threshold ensures OOD signals (Skill A) are strictly ZEROED, preserving Stability.")
    ...
    model.expand_model(
        ...
        router_init_bias=-4.0, # Sigmoid(-4) = 0.018. Below Threshold (0.05).
        gating_threshold=0.05 # V24 addition
    )
