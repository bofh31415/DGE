...
def run_v7_kickstart():
    print(f"\n" + "="*60)
    print(f"🚀 RUNNING V7 KICKSTART: High LR (1e-3) + Low Sparsity (0.01)")
    print("="*60)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_name = f"dge_v7_kickstart_{timestamp}"
    ...
    # 3. Optimizer with HIGH LR
    print(f"Step 3: Optimizer Setup (Router LR=1e-3)...")
    router_params = []
    default_params = []
    ...
    # Use 1e-3 for Router/Gates (Boost)
    # Use 1e-3 for Weights (Standard)
    # We want to wake up the sidecar.
    optimizer = DGEAdamW([
    ...
    print(f"✅ V7 Experiment Complete. Logs in {exp_dir}")
