...
def run_v5_verification():
    print(f"\n" + "="*60)
    print(f"🚀 RUNNING V5 VERIFICATION: Zero Router Init")
    print("="*60)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_name = f"dge_v5_zero_init_{timestamp}"
    ...
    # 2. Expand with H4 Baseline Settings + Zero Init (Implicit in dge_utils)
    # Baseline Config: S=0.05, B=-4.0
    ...
    print(f"Step 3: Optimizer Setup (Router LR=1e-4)...")
    router_params = []
    default_params = []
    ...
    print(f"Step 4: Training Skill B (Sparsity=0.05)...")
    train_task(
    ...
    print(f"✅ V5 Experiment Complete. Logs in {exp_dir}")
