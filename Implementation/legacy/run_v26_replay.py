...
def train_task(model, task_type, vocab_size, steps, logger, start_step, optimizer, probe_task_type=None, replay_task_type=None, replay_ratio=0.1):
    model.train()
    ...
    for i in range(steps):
        try:
            # 1. Primary Batch
            x, y = get_batch(task_type, vocab_size, batch_size=batch_size, device=device)
            
            # 2. Replay Mixing
            if replay_task_type and replay_ratio > 0.0:
                 # Determine buffer size
                 # E.g. 10% of 32 = 3.2 -> 3 samples.
                 replay_size = int(batch_size * replay_ratio)
                 if replay_size > 0:
                      # Generate Replay Batch
                      rx, ry = get_batch(replay_task_type, vocab_size, batch_size=replay_size, device=device)
                      # Replace last 'replay_size' elements of primary batch
                      x[-replay_size:] = rx
                      y[-replay_size:] = ry
            ...
            try:
                ...
                if gate_module and hasattr(gate_module, 'router') and gate_module.router is not None:
                    ...
                    elif hasattr(gate_module.router, 'mlp'): # Bigram
...
def run_experiment():
    ...
    run_name = f"dge_v26_replay_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # 2. Expand: V26 Replay Configuration (MLP Bigram + 10% Replay)
    print(f"Step 2: Expanding Model (Bigram Router, Replay Strategy)...")
    ...
    model.expand_model(
        ...
        router_type='bigram', 
        use_orthogonal_init=False, # V26 Update: Zero Init to avoid Replay gradient conflict
    ...
    # 4. Train Skill B (Count Down) + 10% Replay of Skill A
    print(f"Step 4: Training Skill B (Count Down) + 10% Skill A Replay...")
    train_task(
        ...
        replay_task_type=TaskType.COUNT_UP, # Source of Replay
        replay_ratio=0.1 # 10% mixing
    )
